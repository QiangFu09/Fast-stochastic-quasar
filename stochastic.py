# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 15:53:08 2022

LDS with random perturbation
"""
...
label = simulate(true_cat, feature, h0s, no_grad=True) + 0.1 * torch.randn(b, T) #label
...
class ASGD_options:
    def __init__(self):
        ...
        self.tol_type = 'func'
        self.lr = 1e-6 #1/L 1e-6 for QASGD and 1e-5 for SGD
    ...
    def online_choose(self):
        fea = torch.randn(1, 500)
        fea1 = feature.view(-1)
        label = simulate(true_cat, fea, h0s, no_grad=True)
        return fea1, label
def ssq(v):
...
'''
def Hingeloss(x, feature, label, alpha, func_only=False, grad_only=False):#without regularizer
...
def SGD(fg, x0, options=ASGD_options()):
    ...
    for k in range(K):
        ...
        z_new = z - stepsize * stochastic_grad
...
'''
QASGD for general quasar-convex functions
'''
...
'''
with open("stoSGDLDS51e-6gamma=0.5+", "wb") as fp: 
    pickle.dump(data, fp)
'''
